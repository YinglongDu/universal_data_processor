...
def universal_bbox2yolo(data_path,label_dict=None,train_test_val_ratio=[]):
    ...
    if label_dict:
        ...
        for image in tqdm.tqdm(img_list):
            label = image.replace("jpg",'txt')      #get's the label corresponding to the current image
            try:
            ...
            for bbox in bboxs:
                ...
                try:
                    ...
                    str_bbox_y = str(classes[tittle])+" "+str(bbox_y[0])+" "+str(bbox_y[1])+" "+str(bbox_y[2])+" "+str(bbox_y[3])+"\n"
